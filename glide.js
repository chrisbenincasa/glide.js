    ...
    // If argument is one of the methods, call that methods with additional arguments
        if (methods[options]) {
            ...
            return this.each(function() {
                var $this         = $(this),
                    ...
                    if (options.pagination) {
                        if (options.bootstrap) {
                            ...
                            if (options.paginationClass !== 'glide-pagination') {
                                $paginationWrapper.addClass(options.paginationClass);
                            }
                            ...
                            $paginationWrapper = $('<div />').addClass(options.paginationClass).appendTo($this).css({
                ...
                var createCaption = function (slide, content, index) {
                    content = $('<div class="slider-caption"><div>' + content + '</div></div>');
                    slide.after(content);
                    if (options.orientation === 'horizontal') {
                        content.css('top', controller.height() );
                    } else {
                        content.css({
                            display: 'none',
                            top: $container.height()
                ...
                //animate function
                var animate = function(direction, animation, slide, clicked) {
                ...
                var handleCaptionAnimation = function(captionIndex, show) {
                    ...
                    if (show) {
                        if (options.customCaption.length !== 0) {
                            ...
                            if (options.captionAnimation === 'fade') {
                                caption.hide().
                                    css('top', '-=' + caption.outerHeight()).
                                    delay(options.captionDelay).
                                    fadeIn(options.fadeSpeed, options.fadeEasing);
                            } else if (options.captionAnimation === 'slide') {
                                caption.delay(options.captionDelay).
                                    show().
                                    animate({top: "-=" + caption.outerHeight()}, options.slideSpeed, options.slideEasing);
                            }
                        }
                    } else {
                        if (options.customCaption.length !== 0) {
                            ...
                            if (options.captionAnimation === 'fade') {
                                caption.fadeOut(options.fadeSpeed / 2, options.fadeEasing, function() {
                                    $(this).css('top', controller.height());
                                });
                            } else if (options.captionAnimation === 'slide') {
                                caption.animate({top: "+=" + caption.outerHeight()}, options.slideSpeed, options.slideEasing, function() {
                                    $(this).hide();
                                });
                            }
                        }
